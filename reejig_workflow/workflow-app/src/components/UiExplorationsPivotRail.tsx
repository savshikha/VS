...
import { UI_EXPLORATION_PIVOT_ITEMS } from "@/data/workflow";
import type { DirectionPivotTag } from "@/components/DirectionsExploredPivotRail";

type Explainer = { title: string; body: string; tags?: DirectionPivotTag[] };

type UiExplorationsPivotRailProps = {
  activeId: string;
...
/**
 * Left vertical pivot for the UI explorations tab (same pattern as directions explored;
 * `UI_EXPLORATION_PIVOT_ITEMS` + `UI_EXPLORATION_EXPLAINERS` in data for copy).
 */
export function UiExplorationsPivotRail({
  activeId,
  onActiveChange,
  openId,
  onOpenChange,
  ...
  return (
    <nav
      ...
      aria-label="UI explorations"
    >
      <ul className="flex w-full min-w-0 flex-col gap-0 px-2.5 py-4 sm:px-3">
        {UI_EXPLORATION_PIVOT_ITEMS.map((item) => {
          const isOpen = openId === item.id;
          const isActive = activeId === item.id;
          const copy = explainers[item.id] ?? { title: item.id, body: "" };
          const panelId = `${groupId}-ui-panel-${item.id}`;
          const bodyId = `${groupId}-ui-body-${item.id}`;

          return (
            <li key={item.id} className="min-w-0">
              <div className="group flex w-full min-w-0 items-stretch gap-0 py-2 pr-0.5">
                <div
                  className={cn(
                      ...
                      {copy.tags && copy.tags.length > 0 ? (
                        ...
                        >
                          {copy.tags.map((tag, ti) => (
                            <li key={`${item.id}-ui-pivot-tag-${ti}`}>
                              <span
                                className={cn(
